...
public class Update extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		ssex.setValue(detailedselect.getSsex());
		...
		setnic.setValue(detailedselect.getSetnic());
		...
		sduration.setValue(detailedselect.getSduration());
		...
		submit.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				Newupdate update = new Newupdate( sname.getText(), (String) ssex.getValue(), sage.getText(), 
						(String) setnic.getValue(), syear.getText(), number, sid.getText(), clno.getText(), 
						(String) sduration.getValue()); 
			
			    Alert alert2 = new Alert(Alert.AlertType.INFORMATION);
			  //设置对话框标题
			    alert2.setTitle("修改情况");
			  //设置内容
				if(update.isInsert == true) {
					alert2.setHeaderText("修改成功");
				}else {
					alert2.setHeaderText("修改失败");
				}
				Optional<ButtonType> result = alert2.showAndWait();
				if(update.isInsert == true && result.get() == ButtonType.OK) {
					alert2.close();
					primaryStage.close();
				}
			}
